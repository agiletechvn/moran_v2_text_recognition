...
class MORN(nn.Module):
    def __init__(self, nc, targetH, targetW, inputDataType='torch.cuda.FloatTensor', maxBatch=256, CUDA=True):
        ...
        self.cnn = nn.Sequential(
            nn.MaxPool2d(2, 2),
            nn.Conv2d(nc, 64, 3, 1, 1), nn.BatchNorm2d(
                64), nn.ReLU(True), nn.MaxPool2d(2, 2),
            nn.Conv2d(64, 128, 3, 1, 1), nn.BatchNorm2d(
                128), nn.ReLU(True), nn.MaxPool2d(2, 2),
    ...
    def forward(self, x, test, enhance=1, debug=False):
        ...
        x_small = nn.functional.interpolate(
            x, size=(self.targetH, self.targetW), mode='bilinear', align_corners=True)
        ...
        if debug:
            ...
            for i in range(x.size(0)):
                ...
                img_offsets = (offsets_grid[i]).view(
                    1, self.targetH, self.targetW).data.cpu().add_(-v_min).mul_(1./(v_max-v_min))
                img_offsets = to_pil_image(img_offsets)
                img_offsets = np.array(img_offsets)
                color_map = np.empty(
                    [self.targetH, self.targetW, 3], dtype=int)
                ...
                cv2.addWeighted(color_map, alpha, img_copy,
                                1-alpha, 0, img_copy)
                ...
                total_img = np.ones(
                    [self.targetH, self.targetW*3+10, 3], dtype=int)*255
                total_img[0:self.targetH, 0:self.targetW] = img
                total_img[0:self.targetH, self.targetW +
                          5:2*self.targetW+5] = img_copy
                total_img[0:self.targetH, self.targetW *
                          2+10:3*self.targetW+10] = img_processed
